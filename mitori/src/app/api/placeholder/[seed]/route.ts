...
const PALETTES = [
  ...
  { sky1:"#E8763A",sky2:"#F4A860",sky3:"#F8CC88",glow:"#FFF0A0",l1:"#6A3018",l2:"#4A200E",l3:"#2E1208" },
  ...
  { sky1:"#C06818",sky2:"#DC9848",sky3:"#F0C07A",glow:"#FAE090",l1:"#7C2C10",l2:"#5C200C",l3:"#3C1408" },
  ...
  { sky1:"#A05820",sky2:"#C88040",sky3:"#E8A868",glow:"#FDE8C0",l1:"#804020",l2:"#602C14",l3:"#401C0C" },
  { sky1:"#608CC0",sky2:"#8CB0D4",sky3:"#B4CDE4",glow:"#EEF4FF",l1:"#508040",l2:"#386828",l3:"#224C18" },
  { sky1:"#D07828",sky2:"#E8A050",sky3:"#F8C87A",glow:"#FAE8A8",l1:"#906020",l2:"#704815",l3:"#4C300C" },
]
...
async function generatePhoto(seed: string): Promise<Buffer> {
  ...
  // 地形レイヤー（手前ほど基準位置が下）
  const terrainLayers = [
    buildTerrainLayer(0.26, h,         0.14, h,          W, H), // 遠山
    buildTerrainLayer(0.42, h + 37,    0.10, h + 37,     W, H), // 中山
    buildTerrainLayer(0.56, h + 73,    0.07, h + 73,     W, H), // 近丘
    buildTerrainLayer(0.70, h + 109,   0.05, h + 109,    W, H), // 手前
  ]
  ...
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      ...
      if (y < t0) {
        ...
        col = lerpRgb(base, atmo, 0.45)  // 遠いほど霞む
